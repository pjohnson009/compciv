...
echo '6.'
cat data-hold/legislators-current.json | jq '.[] | .bio .religion' | sort | uniq -c | sort -rn
echo '7.'
cat data-hold/legislators-current.json | jq --raw-output '.[] .terms[0] .start' | sort | cut -d '-' -f 1 | grep -c '2015'
echo '8.'
cat data-hold/legislators-current.json | jq --raw-output '.[] .terms[0] .start' | sort | head -n 1 | grep -oE "[[:digit:]][[:digit:]][[:digit:]][[:digit:]]"
echo '9.'
cat data-hold/legislators-current.json | jq --raw-output '.[] .terms[-1:][0].party' | grep -c 'Republican'
echo '10.'
cat data-hold/legislators-current.json | jq --raw-output '.[] .terms[-1:][0].party' | grep -v 'Republican' | grep -v -c 'Democrat'
echo '11.'
